...
- **`documentacion.md`** (la tabla de políticas de seguridad que empezaste en la Actividad 2.5): a día de hoy no incluye las rutas de `Review` (Acceso a Datos, Tema 3) ni la de `/ws-actividad` que has construido en este mismo tema. Un documento de seguridad que no se actualiza al mismo ritmo que el código deja de ser de fiar — y eso incluye el que acabas de terminar tú.
