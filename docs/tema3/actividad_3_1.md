...
El puerto `5672` es el protocolo AMQP (por el que va a hablar tu aplicación); el `15672` es una interfaz web de administración — levanta el contenedor (`docker compose -f .devcontainer/docker-compose.yml -p gamevault_devcontainer up -d rabbitmq` desde la terminal integrada, o "Dev Containers: Rebuild Container") y entra en `http://localhost:15672` (usuario y contraseña por defecto: `guest`/`guest`) para ver, más adelante, las colas y los mensajes pasando por ellas en vivo.
