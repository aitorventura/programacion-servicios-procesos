...
Este apartado es distinto a todos los anteriores: no vas a escribir código de producción — vas a aprender a **ver** algo que ya está pasando delante de ti sin que lo hayas notado. Tu aplicación Spring Boot ya es multihilo, aunque nunca hayas escrito `new Thread()`.
...
Y por encima de eso están las abstracciones de Spring que usarás en los próximos apartados (`@Async`, los *listeners* de eventos): delegan toda esta gestión en el framework, para que tú declares "esto debe ejecutarse en otro hilo" sin gestionar el `Thread`/`Runnable`/`ExecutorService` explícitamente.
...
??? tip "Abrir resumen"
    ...
    - El warm-up de una caché tras invalidarla es el caso motivador que construirás con `@EnableAsync`/`@Async` en los próximos apartados.
